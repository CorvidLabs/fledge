...
|---------|-------------|----------|
| `fledge introspect --json` | `{schema_version: 1, name, about, aliases, args: [...], subcommands: [...]}`. Each subcommand recursively has the same shape; each arg has `name, long?, short?, aliases?, help?, required, takes_value, value_name?, global?` (fields marked `?` are omitted when empty — don't assume `aliases`/`help`/`value_name` are always present). Each node's `args` is the **complete set of flags accepted at that level**, including inherited globals from ancestors (marked `global: true`). No need to walk up the parent chain. **Core surface only — plugin commands are dispatched as external subcommands and do not appear here. Use `fledge plugins list --json` for installed plugin verbs.** | First contact with fledge |
| `fledge spec list --json` | `{schema_version: 1, action: "spec_list", specs: [{name, version, status, section_count, required_sections, companions, ...}]}`. Note: list items carry `section_count`/`required_sections` (ints), not a `sections` array — the section-name array is only in `spec show --json` | Orienting to a new codebase |
...
| `fledge spec check --json` | `{schema_version: 1, action: "spec_check", engine, specs: [...], totals: {checked, errors, warnings}, strict}`. `engine` is `"structural"` (built-in) or `"specsync"` (the specsync binary is installed — matches CI). `specs[]` is always present and carries the same per-spec shape either way (`{name, version, status, file_count, section_count, required_count, errors, warnings}`). The `specsync` engine additionally emits `engine_version`, `passed` (bool), top-level `errors`/`warnings` (its authoritative verdict — `specs[].errors` is fledge's structural view), and `stale` | Spec-sync validation as data |
| `fledge spec lint [target] --json` | `{schema_version: 1, action: "spec_lint", strict, model_pass: {requested, ran, skipped_reason, provider, model}, specs: [{name, path, version, status, findings: [{check, severity, layer, section, message}], errors, warnings, ignored, passed}], totals: {linted, errors, warnings, ignored}, passed}`. Gates the quality of the **spec itself**, not code-vs-spec. Layer 1 (`layer: "structural"`) always runs offline: required sections present *and non-empty*, no `TODO`/`TBD`/`FIXME` in any required section (case-insensitive, whole-word, prose only — a backticked citation is fine), integer-or-semver `version`, every `files:` entry present, an acceptance signal + a rejection signal. **Every section check honors the project's configured `required_sections`**; the acceptance/rejection signals resolve to a configured section by name (`Behavioral Examples`/`Examples`/`Acceptance…`, `Error Cases`/`Failures`/`Errors…`), and when a project's section set names no analogue the check degrades to a *warning* naming the `accepts:`/`rejects:` frontmatter escape hatch rather than becoming an unsatisfiable error. Layer 2 (`layer: "model"`) is **opt-in via `--ai`** and model-graded (falsifiable purpose, load-bearing invariants, discriminating signals); without `--ai` it is skipped and `model_pass.skipped_reason` says so. `model_pass.requested` reports what was asked for (`--ai` alone), so `--ai --no-ai` reads `requested: true, ran: false`. `check` ids come from a closed vocabulary; `--ignore <check>` is the human override; `--strict` promotes warnings to errors. Exit 1 on any error | Before handing a spec to an agent loop; as a pre-commit hook or CI gate |
| `fledge ai status --json` | `{schema_version: 1, action: "ai_status", provider, provider_source, model, model_source, host, host_source, api_key_set, api_key_source, cloud_routed}`. All keys are always present (values may be `null`). `provider_source` is `"env" \| "config" \| "default"`. `api_key_set` is a bool; `api_key_source` is `"env" \| "config"` (or `null` when no key is set — `api_key_set` is its `!= null` convenience). `cloud_routed` is a bool — whether requests route to a cloud endpoint (Ollama Cloud / `-cloud` auto-routing) — or `null` when not applicable to the provider. `model`/`host` and their `*_source` may be `null` (host is the `base_url`, shown only when set) | Verifying provider config before invoking the LLM |
...
| `fledge run <task> --json` | `{schema_version: 1, action: "run_task", task, command, exit_code, success, stdout, stderr}` (plus `args` when pass-through args are supplied) | Running a task and capturing its output. **One envelope per executed task** — a task with `deps` emits one for each dependency and then one for itself, concatenated on stdout, so parse it as a JSON *stream* (`jq` and `serde_json::StreamDeserializer` do; a single `json.loads` does not). Forward extra args to the command with `fledge run <task> -- <args…>`. Add `--stream` for a long-running or interactive task: the child's output is mirrored to **stderr** live (never to stdout, so the envelope stream stays clean, and the envelope's `stdout`/`stderr` are still complete) and the child keeps stdin. Per-stream ordering only |
| `fledge run --init --json` | `{schema_version: 1, action: "run_init", file, project_type, files_created}` | Scaffolding a `fledge.toml` |
| `fledge release <bump> --dry-run --json` | `{schema_version: 1, action: "release", dry_run: true, version, no_bump, files_to_bump, will_changelog, will_tag, will_push, tag}`. `<bump>` is a **required** positional (`patch`/`minor`/`major`/an explicit version); omitting it exits 2 | Preview what a release would do |
| `fledge release <bump> --json` | `{schema_version: 1, action: "release", dry_run: false, version, old_version, files_bumped, changelog_updated, commit_created, tag_created, tag, pushed}` | After a real release completes |
| `fledge lanes run <name> --dry-run --json` | `{schema_version: 1, lane, description, total_steps, fail_fast, dry_run: true, steps: [{step, kind, name?, items?}]}`. A task/inline step carries `name`; a `kind: "parallel"` step carries `items: [{kind, name}]` instead of a top-level `name` | Preview lane steps without executing |
...
| `fledge plugins audit --json` | `{schema_version: 1, audit: [{name, version, source, trust_tier, runtime, sandboxed, capabilities: {exec, store, metadata, filesystem, network}, commands, has_lifecycle_hooks}]}`. `sandboxed: true` for wasm plugins; `capabilities.filesystem` is `"none" \| "project" \| "plugin"` (the sandbox scope a plugin may reach — escalating up that ladder); `capabilities.network` is bool | Capability/hook audit |
| `fledge plugins search --json` | `{schema_version: 1, results: [{owner, name, full_name, description, stars, url, topics, trust_tier}]}`. Every `*/search` command shares this entry shape (`full_name` is `owner/name`; `topics` is always present, `[]` when empty). Supports `--trust-tier {official,team,unverified}` for client-side filtering after fetch (e.g. `--trust-tier official` for first-party only) | GitHub search for `fledge-plugin`-tagged repos |
...
| `fledge lanes search --json` | `{schema_version: 1, results: [{owner, name, full_name, description, stars, url, topics, trust_tier}]}` (same entry shape as every `*/search` command) | GitHub search for `fledge-lane`-tagged repos |
...
| `fledge templates search --json` | `{schema_version: 1, results: [{owner, name, full_name, description, stars, url, topics, trust_tier}]}` (same entry shape as every `*/search` command) | GitHub search for `fledge-template`-tagged repos |
...
- `0` success
- `1` user-facing error (bad input, missing file, validation failure, prompt required in non-TTY) — a runtime check inside the command
- `2` usage error from the argument parser (clap): an unknown flag, a bad flag value, or a **missing required positional** (e.g. `fledge release` or `fledge spec show` with no argument). Distinct from `1`, and emitted before the command runs, so a `--json` invocation that fails this way prints nothing on stdout
...
- `fledge spec lint`. Run if you *wrote or edited a spec*. `spec check` says the code matches the spec; `spec lint` says whether the spec is worth matching. Add `--ai` for the model-graded quality pass (opt-in, needs a provider)
