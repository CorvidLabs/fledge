...
| `fledge spec lint [target] --json` | `{schema_version: 1, action: "spec_lint", strict, model_pass: {requested, ran, skipped_reason, provider, model}, specs: [{name, path, version, status, findings: [{check, severity, layer, section, message}], errors, warnings, ignored, passed}], totals: {linted, errors, warnings, ignored}, passed}`. Gates the quality of the **spec itself**, not code-vs-spec. Layer 1 (`layer: "structural"`) always runs offline: required sections present *and non-empty*, no `TODO`/`TBD`/`FIXME` in Purpose or Public API (case-insensitive, whole-word, prose only — a backticked citation is fine), integer-or-semver `version`, every `files:` entry present, an acceptance signal + a rejection signal. Layer 2 (`layer: "model"`) is **opt-in via `--ai`** and model-graded (falsifiable purpose, load-bearing invariants, discriminating signals); without `--ai` it is skipped and `model_pass.skipped_reason` says so. `check` ids come from a closed vocabulary; `--ignore <check>` is the human override; `--strict` promotes warnings to errors. Exit 1 on any error | Before handing a spec to an agent loop; as a pre-commit hook or CI gate |
