...
/// Regression: a consumer of fledge's stderr that stops reading must not take
/// the run down with it.
///
/// This is the mirror failure that actually happens in the field (`2>&1 |
/// head`, a pager the user quits, a log collector that closes), and the one an
/// in-memory sink cannot reproduce: `FailAfter` in the unit tests *returns*
/// `BrokenPipe` from Rust, while a real closed pipe raises `SIGPIPE`, whose
/// default disposition `main` restores on purpose. That killed fledge at exit
/// 141 with nothing on stdout — a task that succeeded reported to the caller
/// as a failed run, with no envelope to inspect, which is exactly what
/// invariant 19 and REQ-run-021 forbid.
#[cfg(unix)]
#[test]
fn cli_run_stream_json_survives_a_stderr_reader_that_exits_early() {
    use std::process::{Command, Stdio};

    let tmp = TempDir::new().unwrap();
    fs::write(
        tmp.path().join("fledge.toml"),
        "[tasks]\nticker = \"sleep 1; echo TICK-1; echo TICK-2; echo TICK-3\"\n",
    )
    .unwrap();

    let mut child = Command::new(cargo_bin())
        .args(["run", "ticker", "--json", "--stream"])
        ...
        .spawn()
        .unwrap();
    // Close the read end of fledge's stderr while the task is still sleeping,
    // so every mirror write after that hits a genuinely broken pipe.
    ...
    assert_eq!(
        output.status.code(),
        Some(0),
        "a task that succeeded must still exit 0 when the live echo breaks; \
         got {:?} (no code at all means a signal — 141/SIGPIPE is the bug)",
        output.status
    );
    let stdout = String::from_utf8(output.stdout).unwrap();
    let parsed: serde_json::Value = serde_json::from_str(&stdout)
    ...
    assert_eq!(parsed["exit_code"].as_i64(), Some(0));
    assert!(
        parsed["stdout"].as_str().unwrap().contains("TICK-3"),
        "capture must stay complete after mirroring broke, got: {stdout}"
    );
}
