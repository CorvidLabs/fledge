...
- `Team` tier (renamed from `Community` in spec v2) classifies personal repos owned by CorvidLabs members. The list starts intentionally tight (`["0xLeif"]`) and grows by PR. A future verified-contributor program could either expand this list or introduce a separate `Verified` tier without breaking the existing one
