...
| `FenceTracker` | (internal) Line-by-line fenced-code tracker (`` ``` `` and `~~~`, longer runs included) shared by every heading scanner |
| `new` | (internal) `FenceTracker` constructor — a tracker with no fence open |
| `is_fenced` | (internal) `FenceTracker` method — true while a line is inside a fenced block, its delimiters included |
| `extract_sections` | (internal) Extract `## Section` headings from a spec body, ignoring headings inside fenced code |
| `extract_section_bodies` | (internal) Extract each `## Section` heading paired with its body (up to the next `## ` heading that is not inside a fence) |
| `extract_purpose` | (internal) Extract the first paragraph of prose under `## Purpose` |
...
| `is_fenced` | `(&mut self, line: &str) -> bool` | True for a fenced line, delimiters included; a fence closes only on the same marker at the same length or longer, with no info string |
...
29. **A `## ` line inside a fenced code block is an example, never a section.** Every heading scanner (`extract_sections`, `extract_section_bodies`, `extract_purpose`) runs one `FenceTracker`, so `` ``` `` and `~~~` blocks — longer runs included, which is how a block quotes a shorter fence — are opaque. Without this a quoted heading fabricates a section, and because required sections resolve by first match the phantom shadows the real one: `missing_section` stops firing for a section that exists only as an example, and a real placeholder after the fence lands in the phantom's body where nothing scans it. Documenting the spec format inside a spec is the normal case, so the fence has to stay an example
30. `spec lint` is read-only; it never mutates the filesystem
...
|---------|------|---------|
| 15 | 2026-08-11 | CHG-0009-fledge-spec-lint-two-layer-quality-gate-for-the-spec-itself-issue-429: Fledge spec lint: two-layer quality gate for the spec itself (issue #429). Review fixes for PR #505: the placeholder and signal checks walk the configured `required_sections`; `--ai --no-ai` reports `requested: true` with a `skipped_reason`; and every heading scanner is fence-aware (new `FenceTracker` in `parse.rs`, shared with `strip_code_spans`), so a `## ` line quoted inside a ``` or ~~~ block no longer fabricates a section |
| 14 | 2026-07-30 | Add `fledge spec lint [target]` (#429) — a quality gate for the spec itself, in two layers. Layer 1 is a deterministic offline pre-pass (required sections present and non-empty, no `TODO`/`TBD`/`FIXME` in any configured required section — matched case-insensitively and on word boundaries, against prose with code spans stripped, via `contains_placeholder_word` — integer-or-semver `version`, every `files:` entry present, an acceptance signal and a rejection signal). Layer 2 is an opt-in model-graded pass (`--ai`) reusing `review`'s provider plumbing, judging falsifiable purpose, load-bearing invariants, and discriminating acceptance/rejection signals. New `src/spec/lint.rs`; new `split_frontmatter` / `extract_section_bodies` in `parse.rs`; new shared `DEFAULT_REQUIRED_SECTIONS`. `--json` emits `{schema_version: 1, action: "spec_lint", strict, model_pass, specs: [...], totals, passed}`. `--ignore <check>` is the human override; `--strict` promotes warnings to errors |
