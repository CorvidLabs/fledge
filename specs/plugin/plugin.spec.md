...
| Export | Visibility | Description |
|--------|------------|-------------|
| `run` | `pub` | Entry point — install, list, remove, or run plugins |
| `resolve_plugin_command` | `pub` | Check if a command name matches an installed plugin |
| `run_lifecycle_hook` | `pub` | Run a named lifecycle hook across all installed plugins |
| `DEFAULT_PLUGINS` | `pub` | Curated list of plugin sources installed by `fledge plugins install --defaults` |
...
13. `fledge plugins install --defaults` (mutually exclusive with a positional source ref) installs every entry in the const `DEFAULT_PLUGINS` array. As of v0.15.2: `fledge-plugin-{github,deps,metrics}`. The earlier set also included `templates-remote` (re-absorbed into core `templates search`/`publish`) and `doctor` (re-absorbed into core `doctor` as the informational `Toolchains` section)
14. The `--defaults` install loop reports per-plugin success/failure and continues on error so a single bad repo doesn't block the rest. Exits non-zero if any plugin failed; the trailing summary lists each failure with its error message
