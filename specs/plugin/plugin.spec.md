...
| 14 | 2026-04-25 | Extend `--json` structured-output guarantees to `fledge plugins create` and `fledge plugins publish`. Both commands now emit machine-readable success envelopes on stdout. Hooks and validation run silently in JSON mode (progress to stderr) so stdout remains a single JSON object. |
